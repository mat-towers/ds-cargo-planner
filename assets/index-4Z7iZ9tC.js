...
`;var c=document.querySelector(`#vehicle-select`),l=document.querySelector(`#material-form`),u=document.querySelector(`#material-type`),d=document.querySelector(`#material-amount`),f=document.querySelector(`#standard-form`),p=document.querySelector(`#order-name`),m=document.querySelector(`#status-message`),h=document.querySelector(`#order-table-body`),g=document.querySelector(`#storage-table-body`),_=document.querySelector(`#order-summary`),v=document.querySelector(`#storage-summary`),y=document.querySelector(`#commit-order-button`),b=1,x,S,C;function w(){c.innerHTML=Object.entries(r).map(([e,t])=>`<option value="${e}">${t[0]} (${t[1]} SCU)</option>`).join(``)}function T(){u.innerHTML=s.map(e=>`<option value="${e}">${e}</option>`).join(``)}function E(){return new a(`Current Order`,[])}function D(){C.setOrderName(p.value.trim())}function O(e,t=!1){m.textContent=e,m.classList.toggle(`error`,t)}function k(){return[Number(document.querySelector(`#box-s`).value),Number(document.querySelector(`#box-m`).value),Number(document.querySelector(`#box-l`).value),Number(document.querySelector(`#box-xl`).value),0,0,0].map(e=>!Number.isFinite(e)||e<0?0:Math.trunc(e))}function A(e){return`${e[0]}, ${e[1]}, ${e[2]}, ${e[3]}`}function j(n){let r=n.getBoxes(),i=e[n.getCargoContent()]||[],a=r.map((e,n)=>({count:e,dimension:t[n],amountPerBox:i[n]})).filter(e=>e.count>0);return a.length?a.map(e=>`${e.dimension}: ${e.count}x${e.amountPerBox}`).join(` | `):`-`}function M(e){return e.getIsMaterial()?j(e):A(e.getBoxes())}function N(e){if(!e.getIsMaterial())return e.getCargoContent();let t=e.getMaterialAmount();return`${e.getCargoContent()} (${t})`}function P(){let e=C.getCargoList();if(!e.length){h.innerHTML=`<tr><td colspan="6">No cargo in current order.</td></tr>`;return}h.innerHTML=e.map((e,t)=>{let n=e.getIsMaterial()?`Material`:`Standard`;return`
        <tr>
          <td>${t+1}</td>
          <td>${N(e)}</td>
          <td>${n}</td>
      ...
      `}).join(``)}function F(){let e=S.getOrders();if(!e.length){g.innerHTML=`<tr><td colspan="4">No orders committed to storage.</td></tr>`;return}g.innerHTML=e.map((e,t)=>{let n=e.getCargoList().map(e=>`<li>${N(e)} | ${e.getIsMaterial()?`Material`:`Standard`} | Boxes: [${M(e)}] | ${e.getOccupiedSpace()} SCU</li>`).join(``);return`
        <tr>
          <td>
            <details>
      ...
      `}).join(``)}function I(){let e=r[x][0],t=S.getVehicleCapacity(),n=S.getUsedStorage(),i=S.getCurrentAvailableStorage();_.textContent=`Current order cargo: ${C.getCargoList().length} | Current order SCU: ${C.getTotalSCU()}`,v.textContent=`Vehicle: ${e} | Capacity: ${t} SCU | Used: ${n} SCU | Available: ${i} SCU | Committed orders: ${S.getOrders().length}`}function L(){P(),F(),I()}function R(e){x=e,S=new o(r[x]),a.resetOrderIds(),C=E(),p.value=``,b=1,O(`Vehicle changed. Storage and order have been reset.`),L()}l.addEventListener(`submit`,e=>{e.preventDefault();try{let e=new i({cargoContent:u.value,isMaterial:!0,amount:Number(d.value)});C.addCargo(e),O(`Material cargo added to current order.`),d.value=``,d.focus(),L()}catch(e){O(e.message,!0)}}),f.addEventListener(`submit`,e=>{e.preventDefault();let t=k();if(t.slice(0,4).reduce((e,t)=>e+t,0)<=0){O(`Add at least one box for standard cargo.`,!0);return}let n=new i({cargoContent:`cargo${b++}`,isMaterial:!1,boxes:t});C.addCargo(n),O(`Standard cargo added to current order.`),f.reset(),L()}),h.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof HTMLElement))return;let n=t.getAttribute(`data-remove-order-cargo-index`);if(n===null)return;let r=Number(n);if(!C.removeCargoAt(r)){O(`Could not remove order entry.`,!0);return}O(`Entry removed from current order.`),L()}),g.addEventListener(`click`,e=>{let t=e.target;if(!(t instanceof HTMLElement))return;let n=t.getAttribute(`data-remove-storage-order-index`);if(n===null)return;let r=Number(n);if(!S.removeOrderAt(r)){O(`Could not remove storage entry.`,!0);return}O(`Entry removed from storage.`),L()}),y.addEventListener(`click`,()=>{if(C.isEmpty()){O(`Current order is empty. Add cargo first.`,!0);return}if(D(),!S.addOrder(C)){O(`Not enough storage for this order.`,!0);return}O(`Order committed to storage.`),C=E(),p.value=``,L()}),c.addEventListener(`change`,()=>{R(c.value)}),p.addEventListener(`input`,()=>{D()}),w(),T(),R(c.value);